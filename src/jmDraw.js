 ...
 * @method addCell
 * @for jmDraw
 * @param {object} shapeType 图型类型名
 * @param {object} option 元素参数，主要为jmcell的参数
 */
jmDraw.prototype.addCell = function(shapeType, option) {
		
	var st = jmDraw.shapeTypes[shapeType];
	if(!st) {
 ...
 */
jmDraw.prototype.maxId = function() {
	var id = 0;
 ...
 */
jmDraw.prototype.isEnabled = function(b) {
 ...
 * @private
 */
jmDraw.prototype.initEvents = function() {
	var self = this;
	//编辑器开始框选
	this.graph.bind('mousedown',function(evt) {
		var _this = self;		
 ...
 */
jmDraw.prototype.selectAll = function(b,exid) {
	this.cells.each(function(i,cell) {
		//如果为排除元素则不改变它的状态
 ...
 * @return {jmCell} 元素对象
 */
jmDraw.prototype.getCell = function(id) {
 ...
 * @for jmDraw
 */
jmDraw.prototype.getCells = function() {
 ...
 */
jmDraw.prototype.getSelectedCells = function() {
	var cells = [];
 ...
 * @return {array} 所有已选择的元素数组 
 */
jmDraw.prototype.getSelectedConnects = function() {
	var lines = [];
...
*/
jmDraw.prototype.moveCells = function(cells,dx,dy) {
 ...
 */
jmDraw.prototype.remove = function(cells) {	
	if(jmUtils.isArray(cells)) {
		for(var i = cells.length - 1;i >= 0;i--) {
			cells[i].remove();
		}	
 ...
 * @for jmDraw
 */
jmDraw.prototype.clear = function() {	
 ...
 */
jmDraw.prototype.regComponent = function(el,option) {
 ...
 */
jmDraw.prototype.regStyle = function(name,style) {
 ...
 * @for jmDraw
 */
jmDraw.prototype.save = function() {
 ...
 * @for jmDraw
 */
jmDraw.prototype.undo = function() {
	if(!this.isEnabled()) return;//不可编辑时，直接退出
 ...
 * @for jmDraw
 */
jmDraw.prototype.redo = function() {
	if(!this.isEnabled()) return;//不可编辑时，直接退出
	//当前索引不为最后一个时方可恢复操作
 ...
 */
jmDraw.prototype.align = function(der) {
 ...
 */
jmDraw.prototype.resize = function(cell) {
	var maxw = 0;
	var maxh = 0;
	//取元素边界
	if(cell) {
 ...
 */
jmDraw.prototype.execute = function(cmd) {
	switch(cmd) {
		case 'zoomOut': {
			this.graph.zoomOut();
			break;
 ...
 */
jmDraw.prototype.toImage = function() {
 ...
 */
jmDraw.prototype.toJSON = function() {
